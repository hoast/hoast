...
Hoast.prototype.process = async function(options) {
  ...
  for (let i = 0; i < this.modules.length; i++) {
    if (Object.prototype.hasOwnProperty.call(this.modules[i], 'before') && typeof (this.modules[i].before) === 'function') {
  ...
  const files = await this.helpers.scanDirectory(this.options.sourceAbsolute, this.expressions, this.options.patternOptions.all)
  ...
  for (let i = 0; i < this.modules.length; i++) {
    if (Object.prototype.hasOwnProperty.call(this.modules[i], 'after') && typeof (this.modules[i].after) === 'function') {
